...
// SubjectOlap
// 内表巡检
func SubjectOlap(app string, db *gorm.DB, fs, bs []map[string]interface{}, tic map[string]interface{}, fullTic []map[string]interface{}) (string, int, int) {
	var all []string
	var olaplist []string
	for _, item := range olapInData {
		olaplist = append(olaplist, fmt.Sprintf("%s.%s", item["dbname"].(string), item["tbname"].(string)))
	}
	// 检查主键索引
	index, diffSum := primaryIndex(db, olaplist)
	// 检查内表倾斜
	tilt, diffSum2 := olaptilt(db, olaplist)
	// 副本数不足
	replons, diffSum3 := olapreplicationNum(db, olaplist)
	// 空表
	null, diffSum4 := olapnil(db, olaplist)
	// 生成结论
	...
	tools.WriteFile(strings.NewReplacer("*", fmt.Sprintf("%s_%s", app, time.Now().Format("20060102"))+"_olap").Replace(util.Read.Server.Loadhtmlglob),
...
// 巡检空表
func olapnil(db *gorm.DB, olaplist []string) (string, int) {
...
ORDER BY
  tablet_count desc`)
	...
	var bbodys [][]string
	for _, m2 := range m {
		if tools.StrInSlice(olaplist, fmt.Sprintf("%s.%s", m2["DB_NAME"].(string), m2["TABLE_NAME"].(string))) {
			continue
		}
...
// 查找副本数不是3的表
func olapreplicationNum(db *gorm.DB, olaplist []string) (string, int) {
	stmt := fmt.Sprintf(`SELECT
  DISTINCT pm.DB_NAME,
  pm.TABLE_NAME,
  pm.REPLICATION_NUM
...
// 内表倾斜巡检
func olaptilt(db *gorm.DB, olaplist []string) (string, int) {
...
// 主键持久化巡检
func primaryIndex(db *gorm.DB, olaplist []string) (string, int) {
	stmt := fmt.Sprintf(`
-- 查找没有开启主键索引落盘的表
...
ORDER BY
	...
	var bbodys [][]string
	for _, m2 := range m {
		if tools.StrInSlice(olaplist, fmt.Sprintf("%s.%s", m2["database_name"].(string), m2["table_name"].(string))) {
			continue
		}
