...
func SubjectNode(app string, db *gorm.DB, fs, bs []map[string]interface{}, tic map[string]interface{}) (string, int, int) {
	// 指标体现
	var all, indi []string
	...
	tools.WriteFile(strings.NewReplacer("*", fmt.Sprintf("%s_%s", app, time.Now().Format("20060102"))+"_node").Replace(util.Read.Server.Loadhtmlglob),
