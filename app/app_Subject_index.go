...
func SubjectIndex(app string, db *gorm.DB, fs, bs []map[string]interface{}) {
	...
	////////////////////////////////////////////////////////
	score := (score_node + score_log + score_avgs + score_conf  + score_olap) / 6
	var conclmsg []string
	if score_log < 100 {
	...
	// 写入文件
	tools.WriteFile(strings.NewReplacer("*", firstPrefix+"_index").Replace(util.Read.Server.Loadhtmlglob),
