...
func initwriteDate() {
	...
	dateitem := regexDates(findDir(loadhtmlglob))
	app := util.P.App
	for _, date := range dateitem {
		availdate = append(availdate,
			availableDates{
				Date: date,
				URL:  fmt.Sprintf("http://%s:%d/%s/%s", util.H.Ip, util.Read.Server.Port+100, app, date),
			})
	}
	availdate = append(availdate,
		availableDates{
			Date: time.Now().Format("20060102"),
			URL:  fmt.Sprintf("http://%s:%d/%s", util.H.Ip, util.Read.Server.Port, app),
		})
	//availdateJson
