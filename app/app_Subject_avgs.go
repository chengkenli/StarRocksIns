...
// SubjectAvgs
// 参数巡检
func SubjectAvgs(app string, db *gorm.DB, fs, bs []map[string]interface{}) (string, int, int) {
	var all []string
	// FE/BE参数检查
	b, avgssum, feavgs, beavgs := avgsinsp(fs, bs)
	//////////////////////////////////
	//////////////////////////////////
	// 在这里进行参数过滤
	if avgsInData != nil {
		for _, avg := range avgsInData {
			feavgs = rmSlicevari(feavgs, avg["key"].(string))
			beavgs = rmSlicevari(beavgs, avg["key"].(string))
		}
	}
	//////////////////////////////////
	//////////////////////////////////
...
<div class="section">
	...
	tools.WriteFile(strings.NewReplacer("*", fmt.Sprintf("%s_%s", app, time.Now().Format("20060102"))+"_avgs").Replace(util.Read.Server.Loadhtmlglob),
...
// 参数巡检
func avgsinsp(fs, bs []map[string]interface{}) ([]string, int, []variable, []variable) {
