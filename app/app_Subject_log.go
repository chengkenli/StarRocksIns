...
func SubjectLog(app string, db *gorm.DB, fs, bs []map[string]interface{}) (string, int, int) {
	var all []string

	// 日志巡检汇总
	b, diffSum := loginsp(0, fs, bs)
...
<div class="section">
	...
	tools.WriteFile(strings.NewReplacer("*", fmt.Sprintf("%s_%s", app, time.Now().Format("20060102"))+"_log").Replace(util.Read.Server.Loadhtmlglob),
		bodyTemplates(statictemplates{
			App:   app,
			Title: "日志巡检",
			//Version: fs[0]["Version"].(string),
			Version:       srversion,
...
func logdiagnosis(logmsg string) bool {
	for _, item := range faillog {
		if strings.Compare(logmsg, item) == 0 {
			return true
		}
	}
	return false
...
// 日志巡检
func loginsp(diffSum int, fs, bs []map[string]interface{}) ([]string, int) {
	...
	for i, f := range fs {
		...
		ot := conn.ConnectSSH(
			&util.ConnSSH{
				...
				Command:        fmt.Sprintf("tail -n %d %s/fe.log", line, felogpath),
			})
		// 设置日志内容
	...
	for i, b := range bs {
		...
		ot := conn.ConnectSSH(
			&util.ConnSSH{
				...
				Port:           22,
				PrivateKeyFile: util.Read.Server.Privatekey,
				Command:        fmt.Sprintf("tail -n %d %s/be.INFO", line, belogpath),
			})
		// 设置日志内容
		b := strings.NewReplacer("\n", "<br>").Replace(ot.(string))
		var tag string
